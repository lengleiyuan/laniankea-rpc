...
public class NetKit {
     ...
     */
    private final static Logger LOGGER = LoggerFactory.getLogger(NetKit.class);
    ...
    public static int getAvailablePort(String host, int port, int maxPort) {
        if (isAnyHost(host)
            ...
            throw new KearpcException("Can't bind to ANY port of " + host + ", please check config");
        } else {
            throw new KearpcException("The host " + host
        ...
        if (registryIp != null && registryIp.length() > 0) {
            ...
            for (int i = 0; i < addrs.size(); i++) {
                ...
                if (address != null) {
                    ...
                    if (host != null && !NetKit.isInvalidLocalHost(host)) {
                        return host;
                    }
                }
            }
        }
        if (NetKit.isInvalidLocalHost(host)) {
            host = NetKit.getLocalIpv4();
        }
        return host;
    }
